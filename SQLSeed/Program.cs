    ...
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            if (users != null)
            {
                ...
                {
                    string sql = "SET IDENTITY_INSERT PostsAPISchema.Users ON;"
                                    + "INSERT INTO PostsAPISchema.Users (UserId"
                                    + ",FirstName"
                                    + ",LastName"
                                    + ",Email"
                                    + ",Gender"
                    ...
                    foreach (Users singleUser in users)
                    {
                        ...
                        if ((sql + sqlToAdd).Length > 4000)
                        {
                            ...
                            sql = "SET IDENTITY_INSERT PostsAPISchema.Users ON;"
                                    + "INSERT INTO PostsAPISchema.Users (UserId"
                                    + ",FirstName "
                                    + ",LastName"
                                    + ",Email"
            ...
            dataContextDapper.ExecuteSQL("SET IDENTITY_INSERT PostsAPISchema.Users OFF");
            ...
            dataContextDapper.ExecuteSQL("TRUNCATE TABLE PostsAPISchema.UserSalary");

            if (userSalary != null)
            {
                using (IDbConnection dbConnection = new SqlConnection(config.GetConnectionString("DefaultConnection")))
                {
                    string sql = "INSERT INTO PostsAPISchema.UserSalary (UserId"
                                    + ",Salary)"
                                    + "VALUES";
                    foreach (UserSalary singleUserSalary in userSalary)
                    {
                        ...
                        {
                            dataContextDapper.ExecuteProcedureMulti(sql.Trim(','), dbConnection);
                            sql = "INSERT INTO PostsAPISchema.UserSalary (UserId"
                                    + ",Salary)"
                                    + "VALUES";
                        }
                        sql += sqlToAdd;
            ...
            dataContextDapper.ExecuteSQL("TRUNCATE TABLE PostsAPISchema.UserJobInfo");

            if (userJobInfo != null)
            {
                using (IDbConnection dbConnection = new SqlConnection(config.GetConnectionString("DefaultConnection")))
                {
                    string sql = "INSERT INTO PostsAPISchema.UserJobInfo (UserId"
                                    + ",Department"
                                    + ",JobTitle)"
                                    + "VALUES";
                    foreach (UserJobInfo singleUserJobInfo in userJobInfo)
                    {
                        ...
                        {
                            dataContextDapper.ExecuteProcedureMulti(sql.Trim(','), dbConnection);
                            sql = "INSERT INTO PostsAPISchema.UserJobInfo (UserId"
                                    + ",Department"
                                    + ",JobTitle)"
                                    + "VALUES";
                        }
